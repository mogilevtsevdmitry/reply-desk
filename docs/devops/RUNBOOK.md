...
**Вариант B — повтор конкретной генерации.** Ретраев на уровне BullMQ нет намеренно
(`attempts: 1`); FAILED-генерации не сохраняются (ADR-042: воркер компенсирует лимит,
публикует финальное SSE-событие и удаляет Review). Пользовательский путь — повторная
отправка текста кнопкой «Повторить генерацию» (обычный `POST /api/v1/reviews`). Это
предпочтительный способ «дожать» единичные упавшие генерации.
...
зависшие Generation останутся в PENDING навсегда — их Review нужно удалить руками
(ADR-042: FAILED-строки в БД не хранятся).
